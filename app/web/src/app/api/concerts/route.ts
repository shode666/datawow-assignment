...
import type { ConcertListResponse } from '@/lib/concert';

/** เฉพาะ param ที่ NestJS รู้จัก — ไม่ส่งของแปลกปลอมต่อ */
const FORWARDED = ['page', 'pageSize'] as const;
...
export async function GET(request: NextRequest) {
  ...
  const result = await callApi<ConcertListResponse>(
...
export async function POST(request: NextRequest) {
  ...
  const result = await callApi('/concerts', {
    method: 'POST',
